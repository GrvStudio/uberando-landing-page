...
const teamMembers: TeamMember[] = [
    { id: 1, name: 'NOR CURTIS-LEWIS', position: 'Head of Finance', img: 'https://i.pravatar.cc/300' },
...
const MeetTheTeam: React.FC = () => {
    ...
    return (
        <div className="bg-gray-900 text-white py-12 px-6 md:px-12">
            <h2 className="text-[67px] text-center sm:text-5xl md:text-6xl lg:text-7xl font-bold mb-4 hanken-grotesk-legendary-400">
                Meet The Team
            </h2>
            <Slider {...settings}>
                {teamMembers.map((member) => (
                    <div key={member.id} className="p-4">
                        <div className="flex flex-col items-center">
                            ...
                            <h3 className="text-xl md:text-2xl lg:text-3xl font-bold mb-2">{member.name}</h3>
                            <p className="hanken-grotesk-legendary-400 bg-gradient-to-r inline from-[#2670F6] to-[#00F0C5] bg-clip-text text-transparent">{member.position}</p>
                        </div>
                    </div>
                ))}
            </Slider>
